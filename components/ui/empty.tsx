...
function EmptyHeader({ className, ...props }: React.ComponentProps<'div'>) {
	return <div data-slot="empty-header" className={cn('gap-1 flex max-w-sm flex-col items-center', className)} {...props} />
}
...
function EmptyDescription({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<div
			...
			className={cn('text-xs/relaxed text-muted-foreground [&>a:hover]:text-primary [&>a]:underline [&>a]:underline-offset-4', className)}
			{...props}
		/>
	)
}
